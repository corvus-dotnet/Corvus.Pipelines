Feature: And logical combinator
...
Scenario Outline: One step
	Given I produce the handlers
		...
		| TestHandler  | int        | bool        | sync          | LogicalCombinatorHandlers.And(LogicalCombinatorHandlers.BindPredicateForAnd(Handlers.Handler1)) |
...
Scenario: No steps
	Given I produce the handlers
		| Handler name | Input type | Output type | Sync or async | Handler definition                   |
		| TestHandler  | int        | bool        | sync          | LogicalCombinatorHandlers.And<int>() |
...
Scenario Outline: Two step
	Given I produce the handlers
		...
		| TestHandler  | (bool, bool) | bool        | sync          | LogicalCombinatorHandlers.And(LogicalCombinatorHandlers.BindPredicateForAnd(Handlers.Handler1), LogicalCombinatorHandlers.BindPredicateForAnd(Handlers.Handler2)) |
...
Examples:
	...
	| HandlerState<(bool, bool), bool>.For((false, true))  | false           |
	| HandlerState<(bool, bool), bool>.For((true, false))  | false           |
...
Scenario: Short circuiting
	Given I produce the handlers
		| Handler name | Input type | Output type | Sync or async | Handler definition                                                                                                                                                                                                                                                                                                          |
		| Handler1     | int        | bool        | sync          | state => state.Handled(false)                                                                                                                                                                                                                                                                                               |
		| Handler2     | int        | bool        | sync          | state => state                                                                                                                                                                                                                                                                                                              |
		| TestHandler  | int        | bool        | sync          | LogicalCombinatorHandlers.And(LogicalCombinatorHandlers.BindPredicateForAnd(Handlers.Handler1).Log(logLevel: LogLevel.Warning, name: "Handler1"), LogicalCombinatorHandlers.BindPredicateForAnd(Handlers.Handler2).Log(logLevel: LogLevel.Warning, name: "Handler2")).Log(logLevel: LogLevel.Warning, name: "TestPipeline") |
...
Scenario: Predicate does not call Handled
	Given I produce the handlers
		...
		| TestHandler  | int        | bool        | sync          | LogicalCombinatorHandlers.And(LogicalCombinatorHandlers.BindPredicateForAnd(Handlers.Handler1)).Catch((HandlerState<int, bool> state, InvalidOperationException x) => { state.Logger.LogError(x.Message); return state.Handled(false); }) |
	And I create the service instances
		| Service type | Instance name | Factory method   |
		| TestLogger   | Logger        | new TestLogger() |
	When I execute the sync handler "TestHandler" with the input of type "HandlerState<int, bool>" HandlerState<int,bool>.For(42, Services.Logger)
